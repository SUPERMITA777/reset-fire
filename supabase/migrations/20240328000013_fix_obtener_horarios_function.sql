...
BEGIN
    ...
    IF v_max_clientes IS NULL THEN
        RAISE NOTICE 'Tratamiento no encontrado: %', p_tratamiento_id;
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'El tratamiento no tiene horarios configurados: %', p_tratamiento_id;
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'La fecha % no está disponible para el tratamiento % y box %', p_fecha, p_tratamiento_id, p_box_id;
        RETURN;
    END IF;
