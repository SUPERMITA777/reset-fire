...
BEGIN
    ...
    -- Verificar que el tratamiento tenga horarios configurados
    IF NOT EXISTS (
        SELECT 1 
        FROM horarios_tratamientos 
        WHERE tratamiento_id = p_tratamiento_id
    ) THEN
        RAISE EXCEPTION 'El tratamiento no tiene horarios configurados';
    END IF;

    -- Verificar que la fecha esté dentro de un rango disponible
    IF NOT EXISTS (
        SELECT 1 
        ...
        RAISE EXCEPTION 'La fecha seleccionada no está disponible para este tratamiento y box';
    END IF;
    ...
    IF v_es_compartido THEN
        ...
        WITH turnos_existentes AS (
            SELECT 
                tc.hora_inicio,
                COUNT(*)::INTEGER as clientes_actuales
        ...
        SELECT 
            ht.hora_inicio,
            ht.hora_fin,
            GREATEST(0, v_max_clientes - COALESCE(te.clientes_actuales, 0))::INTEGER as vacantes_disponibles
    ...
    ELSE
        ...
        WITH citas_existentes AS (
            SELECT 
                hora_inicio,
                COUNT(*)::INTEGER as citas_actuales
            FROM citas
            WHERE fecha = p_fecha
                AND box_id = p_box_id
        ...
        SELECT 
            ...
            CASE 
                ...
                ELSE 0
            END::INTEGER as vacantes_disponibles
        FROM horarios_tratamientos ht
